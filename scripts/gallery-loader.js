...
class GalleryLoader {
  ...
  renderGallery() {
    ...
    grid.innerHTML = '';
    const allData = this.getGalleryData();
    const galleryFragment = Core.DOM.createFragment(images, (img, idx) => this.createGalleryItem(img, idx, allData));
  ...
  createGalleryItem(image, index, allData) {
    // Delegate to Core.Media to ensure consistent behavior across app
    const data = allData || this.getGalleryData();
