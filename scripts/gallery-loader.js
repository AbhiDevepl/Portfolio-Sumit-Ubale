...
class GalleryLoader {
  ...
  renderGallery() {
    ...
    grid.innerHTML = '';
    // Hoist data aggregation out of the loop for O(N) performance
  ...
  createGalleryItem(image, index, galleryData) {
    // Use provided galleryData if available, otherwise fallback to getGalleryData() for robustness
    const items = galleryData || this.getGalleryData();
