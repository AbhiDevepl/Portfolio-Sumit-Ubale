...
class GalleryLoader {
  ...
  async init() {
    ...
    try {
      ...
      // Performance optimization: Pre-calculate category name lookup map
      this.categoryNames = {};
  ...
  renderGallery() {
    ...
    // Performance optimization: Hoist getGalleryData() out of the loop.
    // This avoids rebuild of the entire gallery data array for every single item (O(N^2) bottleneck).
  ...
  /**
   * Optimized: now accepts optional pre-computed galleryData to avoid O(N^2) rebuilding in loops
   */
  createGalleryItem(image, index, galleryData = null) {
    // Delegate to Core.Media to ensure consistent behavior across app
    // Optimized: using pre-calculated categoryNames map for O(1) lookups
