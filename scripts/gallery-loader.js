...
class GalleryLoader {
  ...
  renderGallery() {
    ...
    // Performance Optimization: Cache gallery data once instead of re-calculating in every loop iteration (O(N^2) -> O(N))
  ...
  createGalleryItem(image, index, galleryData) {
    ...
    // galleryData is passed from renderGallery to avoid redundant O(N) calculations
