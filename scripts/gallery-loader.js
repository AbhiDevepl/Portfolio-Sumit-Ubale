...
class GalleryLoader {
  ...
  renderGallery() {
    ...
    // Aggregate images (Pre-calculate for O(N) performance instead of O(N^2) in the loop)
    ...
    grid.innerHTML = '';
    // Use the pre-calculated galleryData for both rendering and lightbox data
    const galleryFragment = Core.DOM.createFragment(galleryData, (img, idx) => {
      return Core.Media.createItem(img, idx, galleryData, (cat) => this.category);
    });
