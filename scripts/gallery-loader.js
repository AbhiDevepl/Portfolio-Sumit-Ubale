...
class GalleryLoader {
  ...
  renderGallery() {
    ...
    // Performance: Pre-calculate gallery data once outside the loop
  ...
  createGalleryItem(image, index, galleryData = null) {
    // Performance: Avoid repeated getGalleryData() calls in loops
    const itemsForLightbox = galleryData || this.getGalleryData();

    // Delegate to Core.Media to ensure consistent behavior across app
    return Core.Media.createItem(image, index, itemsForLightbox, (cat) => this.category);
  }
