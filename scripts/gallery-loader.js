...
class GalleryLoader {
  ...
  renderGallery() {
    ...
    // Caching this result prevents O(N^2) complexity during the rendering loop
  ...
  createGalleryItem(image, index, galleryData) {
    ...
    // Pass cached galleryData to avoid redundant calculations
    return Core.Media.createItem(image, index, galleryData || this.getGalleryData(), (cat) => this.category);
  }
