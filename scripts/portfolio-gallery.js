...
class GalleryRenderer {
  ...
  createGalleryItem(item, index) {
    ...
    overlay.innerHTML = `
      ...
      <p class="gallery-item-category">${item.formattedCategory || this.formatCategoryName(item.category)}</p>
    `;
...
class PortfolioGallery {
  ...
  processData(data) {
    ...
    categoryOrder.forEach((cat, index) => {
      categoryWeights[cat] = index;
    });

    // Process and flatten with pre-calculated sort keys (Schwartzian Transform)
    // NOTE: Object.assign and manual logical checks used for Cloudflare CI compatibility
    ...
    for (let i = 0; i < categories.length; i++) {
      ...
      if (Array.isArray(items)) {
        // Unknown categories get weight -1 to match original indexOf behavior
        const weight = categoryWeights[category] !== undefined ? categoryWeights[category] : -1;
        ...
        for (let j = 0; j < items.length; j++) {
          ...
          // Store weight and order directly on the item for O(1) comparison
    ...
    // Sort using pre-calculated weights and orders
    allItems.sort((a, b) => {
