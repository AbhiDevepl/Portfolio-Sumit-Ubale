...
class GalleryState {
  ...
  setFilteredList(items) {
    ...
    // While notify() is part of the state pattern, explicit render call here
    // ensures the renderer stays in sync with filtered list changes.
...
class PortfolioGallery {
  ...
  processData(data) {
    ...
    if (!PortfolioGallery.CATEGORY_WEIGHTS) {
      PortfolioGallery.CATEGORY_WEIGHTS = {};
      const order = PortfolioGallery.CATEGORY_ORDER;
      for (let i = 0; i < order.length; i++) {
        PortfolioGallery.CATEGORY_WEIGHTS[order[i]] = i;
      }
    }

    // Initialize category name cache
    if (!this.categoryNameCache) {
      this.categoryNameCache = {};
    }

    // Optimized processing loop: avoid O(N) operations inside loops
    ...
    for (let i = 0; i < categories.length; i++) {
      ...
      if (Array.isArray(items)) {
        ...
        if (!this.categoryNameCache[category]) {
          this.categoryNameCache[category] = this.formatCategoryName(category);
        }
        const categoryName = this.categoryNameCache[category];
        const categoryWeight = PortfolioGallery.CATEGORY_WEIGHTS[category] !== undefined
          ? PortfolioGallery.CATEGORY_WEIGHTS[category]
          : 1000;

        for (let j = 0; j < items.length; j++) {
          const item = items[j];
          const title = item.title || (categoryName + ' ' + (j + 1));
    ...
    // Optimized sort: uses pre-calculated O(1) weights and orders
    allItems.sort((a, b) => {
...
// ========================================
// Using property assignment after class definition for Cloudflare CI compatibility
...
PortfolioGallery.CATEGORY_WEIGHTS = null;
