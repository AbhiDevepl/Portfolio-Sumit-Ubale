...
class GalleryRenderer {
  ...
  openLightbox(index) {
    ...
    const lightboxItems = items.map((item, i) => {
      ...
      cloned.type = item.type || 'image';
      cloned.originalIndex = i;
      return cloned;
    });
...
class PortfolioGallery {
  ...
  processData(data) {
    ...
    for (let i = 0; i < categoryOrder.length; i++) {
      categoryWeights[categoryOrder[i]] = i;
    }

    const formatCache = {};
    const formatCategory = (slug) => {
      if (formatCache[slug]) return formatCache[slug];
      formatCache[slug] = this.formatCategoryName(slug);
      return formatCache[slug];
    };
    ...
    for (let c = 0; c < categories.length; c++) {
      ...
      if (Array.isArray(items)) {
        ...
        for (let i = 0; i < items.length; i++) {
          ...
          newItem.title = item.title || (formattedCategory + ' ' + (i + 1));
