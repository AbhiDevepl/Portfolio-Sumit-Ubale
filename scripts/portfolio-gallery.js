...
class GalleryRenderer {
  ...
  createGalleryItem(item, index) {
    ...
    // Use pre-calculated formatted category
...
class PortfolioGallery {
  constructor() {
    ...
    var self = this;
    PortfolioGallery.CATEGORY_ORDER.forEach(function(cat, idx) {
      self._categoryWeights[cat] = idx;
    });
  ...
  processData(data) {
    ...
    // Pre-calculate total items to minimize array allocations
    var totalLen = 0;
    categories.forEach(function(cat) {
      if (Array.isArray(images[cat])) totalLen += images[cat].length;
    });

    var allItems = new Array(totalLen);
    var pointer = 0;

    // Single-pass flattening and enrichment
    for (var i = 0; i < categories.length; i++) {
      ...
      if (Array.isArray(items)) {
        ...
        var weight = this._categoryWeights[category] !== undefined ? this._categoryWeights[category] : 999;
        ...
        for (var j = 0; j < items.length; j++) {
          ...
          newItem.title = item.title || (formattedCat + ' ' + (j + 1));
          newItem.alt = item.alt || item.title || (formattedCat + ' photography');
          newItem.type = item.type || 'image';
          allItems[pointer++] = newItem;
        }
      }
    }

    // Sort using pre-calculated weights O(1) vs O(M) indexOf
