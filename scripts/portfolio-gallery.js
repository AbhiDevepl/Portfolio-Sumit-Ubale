...
class GalleryRenderer {
  ...
  createGalleryItem(item, index) {
    ...
    categoryEl.textContent = item.categoryLabel || this.formatCategory(item.category);
  ...
  processData(data) {
    ...
    for (const [category, items] of Object.entries(images)) {
      if (Array.isArray(items)) {
        ...
        const weight = weights[category] !== undefined ? weights[category] : 100;
        ...
        items.forEach((item, index) => {
          allItems.push({
            ...
            title: item.title || `${categoryLabel} ${index + 1}`,
            alt: item.alt || item.title || `${categoryLabel} photography`,
            type: item.type || 'image'
          });
        });
      }
    ...
    // Sort by pre-calculated category weight (O(1) lookup), then by item order
    allItems.sort((a, b) => {
