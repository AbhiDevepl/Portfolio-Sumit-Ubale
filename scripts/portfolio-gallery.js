...
// GALLERY RENDERER
// ========================================
function GalleryRenderer(state, container) {
  this.state = state;
...
GalleryRenderer.prototype.formatCategory = function(category) {
  if (!category) return '';
  // Use shared cache if available from global instance
  if (window.PortfolioGallery && window.PortfolioGallery._categoryNameCache && window.PortfolioGallery._categoryNameCache[category]) {
    return window.PortfolioGallery._categoryNameCache[category];
  }
  return category
    .split('-')
...
ModalViewer.prototype.navigate = function(direction) {
  ...
  var stateObj = (window.PortfolioGallery && window.PortfolioGallery.state) ? window.PortfolioGallery.state.getState() : null;
  if (!stateObj) return;

  var items = stateObj.filteredList.length > 0 ? stateObj.filteredList : stateObj.mediaList;
...
ModalViewer.prototype.open = function(index) {
  ...
  var stateObj = this.state.getState();
  var items = stateObj.filteredList.length > 0 ? stateObj.filteredList : stateObj.mediaList;
...
// FILTER CONTROLLER
// ========================================
function FilterController(state, chipsContainer) {
  this.state = state;
...
FilterController.prototype.filterByCategory = function(category) {
  var stateObj = this.state.getState();
  var allItems = stateObj.mediaList;
...
FilterController.prototype.selectFromURL = function() {
  ...
  if (category) {
    ...
    var chip = null;
    for (var i = 0; i < chips.length; i++) {
      if (chips[i].getAttribute('data-category') === category) {
        chip = chips[i];
        break;
      }
    }
...
function PortfolioGallery() {
  ...
  this._initCategoryWeights();

  this.init();
}

PortfolioGallery.prototype._initCategoryWeights = function() {
  var categoryOrder = [
    'weddings',
...
PortfolioGallery.prototype.setup = async function() {
  // Get container
...
PortfolioGallery.prototype.fetchData = async function() {
...
PortfolioGallery.prototype.processData = function(data) {
  ...
  // Pre-calculate total items to minimize array re-allocations
  var totalItems = 0;
  for (var i = 0; i < categories.length; i++) {
    if (Array.isArray(images[categories[i]])) {
      totalItems += images[categories[i]].length;
    }
  }

  var allItems = new Array(totalItems);
  var k = 0;

  // Flatten all category images and enrich in a single pass
  for (var i = 0; i < categories.length; i++) {
    var category = categories[i];
    var items = images[category];
    if (Array.isArray(items)) {
      var catWeight = this._categoryWeights[category] !== undefined ? this._categoryWeights[category] : 999;
  ...
  // Sort using pre-calculated weights O(1) lookups instead of O(M) indexOf
