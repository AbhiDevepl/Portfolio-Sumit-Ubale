...
class ContentLoader {
  ...
  renderCategory(category) {
    ...
    items.forEach((item, index) => {
      ...
      el.addEventListener('click', () => {
        const visibleItems = (window.GalleryManager && typeof window.GalleryManager.getVisibleData === 'function' ? window.GalleryManager.getVisibleData() : null) || items;
  ...
  populateAbout() {
    ...
    Object.entries(sections).forEach(([id, className]) => {
      ...
      const data = this.data && this.data.socialProof && this.data.socialProof[id];
      if (container && data) {
        container.innerHTML = '';
        data.forEach(text => {
