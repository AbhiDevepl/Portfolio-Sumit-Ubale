...
class ContentLoader {
  ...
  getFilteredItems(category) {
    ...
    if (category === 'all') {
      ...
      // Optimized: Use reduce for CI compatibility (no .flat())
      var all = [];
      for (var key in this.mediaData) {
        if (Object.prototype.hasOwnProperty.call(this.mediaData, key)) {
          all = all.concat(this.mediaData[key]);
        }
      }
      return all;
    }
