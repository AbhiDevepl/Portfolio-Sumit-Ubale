...
class ContentLoader {
  ...
  populateGallery() {
    ...
    if (Array.isArray(rawImages)) {
      ...
      const sortKeyRegex = /(\d+)\.(jpe?g|mp4|mov)$/i;

      Object.entries(rawImages).forEach(([categorySlug, images]) => {
        // 1. Schwartzian Transform: Pre-calculate sort keys and types to avoid redundant parsing
        const mapped = [];
        for (let i = 0, len = images.length; i < len; i++) {
          ...
          if (isJpg || isVideo) {
            const match = urlWithoutParams.match(sortKeyRegex);
        ...
        // 2. Local Sort numerically based on pre-calculated sortKey
        mapped.sort((a, b) => a.sortKey - b.sortKey);

        // 3. Transform back and assign order
        for (let i = 0, len = mapped.length; i < len; i++) {
          const item = mapped[i];
          allImages.push({
            ...item.img,
            ...
            type: item.type,
            order: i,
            _sortKey: item.sortKey // Stored for final global sort
          });
        }
      });

      // 4. Final Global Sort by pre-calculated filename number
      allImages.sort((a, b) => {
        if (a._sortKey !== b._sortKey) return a._sortKey - b._sortKey;
        return a.src.localeCompare(b.src);
      });

      // Cleanup temporary sort keys to keep data clean
