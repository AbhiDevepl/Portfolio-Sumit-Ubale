  ...
  populateGallery() {
    ...
    if (Array.isArray(rawImages)) {
      ...
      Object.entries(rawImages).forEach(([categorySlug, images]) => {
        // 1. Filter and pre-calculate types/sort keys (Schwartzian Transform)
        const validImages = images
          .map(img => {
            ...
            if (categorySlug === 'cinematics') {
              if (isJpg || isVideo) type = isVideo ? 'video' : 'image';
            } else if (isJpg) {
              type = 'image';
            }
            ...
            // Extract numeric key once
            const match = urlWithoutParams.match(/(\d+)\.(jpe?g|mp4|mov)$/i);
            const sortKey = match ? parseInt(match[1], 10) : 0;

            return { ...img, type, sortKey };
          })
          .filter(img => img !== null);

        // 2. Sort numerically based on pre-calculated sortKey
        validImages.sort((a, b) => a.sortKey - b.sortKey);
      ...
      // 4. Final Global Sort using pre-calculated sortKey
      allImages.sort((a, b) => {
    ...
    // Store processed images for global access (e.g. GalleryManager)
    this.allImages = allImages;

    // Create gallery items using DocumentFragment for performance
    // Pass skipHandler: true to use event delegation in GalleryManager
