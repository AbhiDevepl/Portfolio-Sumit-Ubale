...
class ContentLoader {
  ...
  populateGallery() {
    ...
    // Internal helper for optimized sort key extraction
    const _extractSortInfo = (img) => {
      ...
      return {
        ...
        isVid: srcWithoutParams.toLowerCase().endsWith('.mp4') || srcWithoutParams.toLowerCase().endsWith('.mov')
      };
    };
    ...
    if (Array.isArray(rawImages)) {
      ...
      Object.entries(rawImages).forEach(([categorySlug, images]) => {
        // 1. Optimize: Map to include sort info once (Schwartzian Transform) and filter
        const mapped = images
          .map(img => {
            ...
            const isVid = srcWithoutParams.endsWith('.mp4') || srcWithoutParams.endsWith('.mov');
            if (!isJpg && !isVid) return null;

            return { img, info: _extractSortInfo(img) };
          })
          .filter(Boolean);
        ...
        mapped.forEach((item, idx) => {
          allImages.push({
            ...
            type: item.img.type || (item.info.isVid ? 'video' : 'image'),
            order: idx, // used for pagination logic later
            _sortNum: item.info.num // Temporary for global sort
          });
        });
      });
    ...
    // Assign global index and expose data for O(1) retrieval in GalleryManager
    this.allImages = allImages.map(({_sortNum, ...img}, idx) => ({
      ...img,
      globalIndex: idx
    }));
