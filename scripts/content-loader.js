...
class ContentLoader {
  ...
  getFilteredItems(category) {
    ...
    if (category === 'all') {
      // Flatten all category arrays manually (no .flat())
      var acc = [];
      var keys = Object.keys(this.mediaData);
      for (var i = 0; i < keys.length; i++) {
        var val = this.mediaData[keys[i]];
  ...
  renderCategory(category) {
    ...
    this.allImages = [];
    for (var i = 0; i < items.length; i++) {
      this.allImages.push(Object.assign({}, items[i], { originalIndex: i }));
    }
    if (window.GalleryManager) {
  ...
  populateAbout() {
    ...
    if (publicationsContainer && this.data && this.data.socialProof && this.data.socialProof.publications) {
      ...
      for (var i = 0; i < this.data.socialProof.publications.length; i++) {
        var pub = this.data.socialProof.publications[i];
    ...
    if (awardsContainer && this.data && this.data.socialProof && this.data.socialProof.awards) {
      ...
      for (var i = 0; i < this.data.socialProof.awards.length; i++) {
        var award = this.data.socialProof.awards[i];
    ...
    if (clientsContainer && this.data && this.data.socialProof && this.data.socialProof.clients) {
      ...
      for (var i = 0; i < this.data.socialProof.clients.length; i++) {
        var client = this.data.socialProof.clients[i];
