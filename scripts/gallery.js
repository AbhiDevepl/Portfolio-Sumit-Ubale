window.GalleryManager = {
  ...
  getVisibleData() {
    return Array.from(document.querySelectorAll('.gallery-item'))
      .filter(item => {
        // Performance: Use offsetParent check for display:none instead of getComputedStyle
        // and only check inline style for opacity if needed, to avoid expensive layout reflows.
        if (item.offsetParent === null) return false;

        // If GSAP is used, we might need to check opacity.
        // We'll check the inline style first, then fallback to getComputedStyle only if necessary.
