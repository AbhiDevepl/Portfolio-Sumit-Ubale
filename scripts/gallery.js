window.GalleryManager = {
  ...
  getVisibleData() {
    return Array.from(document.querySelectorAll('.gallery-item'))
      .filter(item => {
        // Optimization: item.offsetParent === null is a faster way to check for display: none
        // We only fall back to getComputedStyle if needed (e.g. for opacity)
        if (item.offsetParent === null) return false;

        // Only check opacity if needed - usually 1.0 if offsetParent exists
