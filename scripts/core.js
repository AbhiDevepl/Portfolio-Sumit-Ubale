...
(function() {
  // Private module state for request deduplication
  let _portfolioDataPromise = null;
  let _servicesDataPromise = null;

  window.Core = {
    /**
     * CACHED PORTFOLIO FETCH WITH REQUEST DEDUPLICATION
     * Utilizes sessionStorage and in-memory promise caching to completely avoid
     * redundant sessionStorage lookups, JSON parsing, and concurrent duplicate network fetches.
     */
    ...
    /**
     * CACHED SERVICES FETCH WITH REQUEST DEDUPLICATION
     * Utilizes sessionStorage and in-memory promise caching to completely avoid
     * redundant sessionStorage lookups, JSON parsing, and concurrent duplicate network fetches.
     */
