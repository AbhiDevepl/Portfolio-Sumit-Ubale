...
window.Core = {
  Lightbox: {
    ...
    init: function() {
      ...
      const html = '<div id="lightbox" class="lightbox" aria-hidden="true" role="dialog" aria-modal="true" aria-label="Fullscreen gallery preview">' +
          ...
          '<div class="lightbox-content">' +
              ...
              '<div class="lightbox-video-wrapper" style="display: none;">' +
                ...
                '<div class="video-overlay-controls">' +
                  '<button class="video-play-pause" type="button" aria-label="Play or pause video">' +
                    '<svg class="play-icon" width="80" height="80" viewBox="0 0 24 24" fill="white"><path d="M8 5v14l11-7z"/></svg>' +
                    '<svg class="pause-icon" width="80" height="80" viewBox="0 0 24 24" fill="white" style="display: none;"><path d="M6 4h4v16H6V4zm8 0h4v16h-4V4z"/></svg>' +
                  '</button>' +
                '</div>' +
                '<div class="video-controls-bar">' +
                  '<button class="video-control-btn play-pause-small" type="button" aria-label="Play or pause video">' +
                    '<svg class="play-icon-small" width="24" height="24" viewBox="0 0 24 24" fill="white"><path d="M8 5v14l11-7z"/></svg>' +
                    '<svg class="pause-icon-small" width="24" height="24" viewBox="0 0 24 24" fill="white" style="display: none;"><path d="M6 4h4v16H6V4zm8 0h4v16h-4V4z"/></svg>' +
                  '</button>' +
                  ...
                  '<button class="video-control-btn mute-btn" type="button" aria-label="Mute or unmute video">' +
                    '<svg class="volume-icon" width="24" height="24" viewBox="0 0 24 24" fill="white"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02z"/></svg>' +
            ...
            '<div class="lightbox-caption">' +
              '<div class="lightbox-caption-copy"><h3></h3><p></p></div>' +
    ...
    bindEvents: function() {
      ...
      document.addEventListener('keydown', function(e) {
        ...
        if (e.key === ' ') { e.preventDefault(); self.togglePlayPause(); }
      });
    },
    ...
    bindVideoControls: function() {
      ...
      muteBtn.onclick = function() { video.muted = !video.muted; self.updateMuteButton(); };
      ...
      const showControls = function() {
        ...
        controlsTimeout = setTimeout(function() {
          if (!video.paused) container.querySelector('.video-controls-bar').style.opacity = '0';
        }, 3000);
      };
    ...
    updateContent: function() {
      ...
      const container = this.state.container;
      const imgEl = container.querySelector('.lightbox-image');
      const vidWrapper = container.querySelector('.lightbox-video-wrapper');
      const vidEl = container.querySelector('.lightbox-video');
      const loadingEl = container.querySelector('.lightbox-loading');
      const captionTitle = container.querySelector('.lightbox-caption-copy h3');
      const captionCategory = container.querySelector('.lightbox-caption-copy p');
      ...
      requestAnimationFrame(function() {
        if (isVid) {
          ...
          vidEl.onerror = function() { loadingEl.style.display = 'none'; };
        } else {
  ...
  VideoObserver: {
    ...
    observe: function(video) { if (!this.observer) this.init(); this.observer.observe(video); }
  },
  ...
  VideoHover: {
    init: function(videoElement) {
      ...
      if (!parent) return;
      if (window.Core.VideoObserver) window.Core.VideoObserver.observe(videoElement);

      const self = this;
      const play = function() {
        self.stopAllVideos(videoElement);
        if (videoElement.dataset.src && !videoElement.src) videoElement.src = videoElement.dataset.src;
        videoElement.play().catch(function() {});
      };
      const stop = function() { videoElement.pause(); videoElement.currentTime = 0; };
      parent.addEventListener('click', function(e) { if (e.target.closest('.gallery-overlay')) return; videoElement.paused ? play() : stop(); });
      const isTouch = 'ontouchstart' in window || navigator.maxTouchPoints > 0;
      if (!isTouch) { parent.addEventListener('mouseenter', play); parent.addEventListener('mouseleave', stop); }
    },
    stopAllVideos: function(currentVideo) {
      document.querySelectorAll('video').forEach(function(video) {
        if (video !== currentVideo && !video.paused && !video.closest('#lightbox')) video.pause();
      });
    }
  },
    ...
    createItem: function(image, index, allItems, categoryFormatter) {
      ...
      const openFilteredLightbox = function() {
        const fallbackItems = allItems.map(function(entry, entryIndex) {
          ...
          enriched.originalIndex = entryIndex;
          enriched.type = entry.type || 'image';
          return enriched;
        });
        ...
        for(let i=0; i<visibleItems.length; i++) if(visibleItems[i].originalIndex === index) { targetIndex = i; break; }
        if (window.Core && window.Core.Lightbox) window.Core.Lightbox.open(targetIndex, visibleItems);
      };
      ...
      if (isVideo) {
        ...
        requestAnimationFrame(function() { media.style.opacity = '1'; item.classList.remove('loading'); });
      } else {
        media.src = image.src;
        media.loading = 'lazy';
        media.alt = image.title || '';
        media.onload = function() { media.style.opacity = '1'; item.classList.remove('loading'); media.classList.add('loaded'); };
        if (media.complete) { media.style.opacity = '1'; item.classList.remove('loading'); media.classList.add('loaded'); }
      }
      item.appendChild(media);
      if (isVideo && window.Core && window.Core.VideoHover) window.Core.VideoHover.init(media);
      if (isVideo) {
      ...
      if (isVideo) {
          item.onclick = function(e) { if (!e.target.closest('video')) openFilteredLightbox(); };
          overlay.onclick = function(e) { e.stopPropagation(); openFilteredLightbox(); };
      } else {
           item.onclick = function() { openFilteredLightbox(); };
      }
      item.addEventListener('keydown', function(e) { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openFilteredLightbox(); } });
      return item;
    }
  },
  ...
  DOM: {
    createFragment: function(items, renderer) {
      ...
      items.forEach(function(item, idx) { const rendered = renderer(item, idx); if (rendered) fragment.appendChild(rendered); });
      return fragment;
    },
    injectGlobalComponents: function() {
      ...
      if (nav) {
        nav.innerHTML = '<div class="nav-container">' +
            ...
            '</div>' +
            '<button class="nav-toggle" aria-label="Toggle navigation"><span class="nav-toggle-line"></span><span class="nav-toggle-line"></span></button>' +
          '</div>';
      }
