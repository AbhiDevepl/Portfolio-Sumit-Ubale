...
window.Core = {
  ...
  Lightbox: {
    ...
    init: function() {
      ...
      var html = '<div id="lightbox" class="lightbox" aria-hidden="true" role="dialog" aria-modal="true" aria-label="Fullscreen gallery preview"><div class="lightbox-overlay"></div><div class="lightbox-content"><button class="lightbox-close" type="button" aria-label="Close preview">&times;</button><button class="lightbox-prev" type="button" aria-label="Previous item">&#8249;</button><button class="lightbox-next" type="button" aria-label="Next item">&#8250;</button><div class="lightbox-media-container"><div class="lightbox-loading" style="display: none;"><div class="spinner"></div><p>Loading...</p></div><img class="lightbox-image" src="" alt="" style="display: none;"><div class="lightbox-video-wrapper" style="display: none;"><video class="lightbox-video" playsinline controls></video><div class="video-overlay-controls"><button class="video-play-pause" type="button" aria-label="Play or pause video"><svg class="play-icon" width="80" height="80" viewBox="0 0 24 24" fill="white"><path d="M8 5v14l11-7z"/></svg><svg class="pause-icon" width="80" height="80" viewBox="0 0 24 24" fill="white" style="display: none;"><path d="M6 4h4v16H6V4zm8 0h4v16h-4V4z"/></svg></button></div><div class="video-controls-bar"><button class="video-control-btn play-pause-small" type="button" aria-label="Play or pause video"><svg class="play-icon-small" width="24" height="24" viewBox="0 0 24 24" fill="white"><path d="M8 5v14l11-7z"/></svg><svg class="pause-icon-small" width="24" height="24" viewBox="0 0 24 24" fill="white" style="display: none;"><path d="M6 4h4v16H6V4zm8 0h4v16h-4V4z"/></svg></button><div class="video-progress-container"><input type="range" class="video-progress" min="0" max="100" value="0" step="0.1"><div class="video-progress-filled"></div></div><span class="video-time">0:00 / 0:00</span><button class="video-control-btn mute-btn" type="button" aria-label="Mute or unmute video"><svg class="volume-icon" width="24" height="24" viewBox="0 0 24 24" fill="white"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02z"/></svg><svg class="mute-icon" width="24" height="24" viewBox="0 0 24 24" fill="white" style="display: none;"><path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z"/></svg></button><input type="range" class="volume-slider" min="0" max="100" value="100" step="1"><select class="playback-speed" aria-label="Playback speed"><option value="0.5">0.5x</option><option value="0.75">0.75x</option><option value="1" selected>1x</option><option value="1.25">1.25x</option><option value="1.5">1.5x</option><option value="2">2x</option></select><button class="video-control-btn fullscreen-btn" type="button" aria-label="Open video fullscreen"><svg width="24" height="24" viewBox="0 0 24 24" fill="white"><path d="M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z"/></svg></button></div></div></div><div class="lightbox-caption"><div class="lightbox-caption-copy"><h3></h3><p></p></div><div class="lightbox-counter" aria-live="polite"></div></div></div></div>';
    ...
    updateContent: function() {
      ...
      var container = this.state.container;
      var imgEl = container.querySelector('.lightbox-image');
      var vidWrapper = container.querySelector('.lightbox-video-wrapper');
      var vidEl = container.querySelector('.lightbox-video');
      var loadingEl = container.querySelector('.lightbox-loading');
      var captionTitle = container.querySelector('.lightbox-caption-copy h3');
      var captionCategory = container.querySelector('.lightbox-caption-copy p');
      var counterEl = container.querySelector('.lightbox-counter');
  ...
  VideoHover: {
    ...
    stopAllVideos: function(currentVideo) {
  ...
  Media: {
    createItem: function(image, index, allItems, categoryFormatter) {
      ...
      if (isVideo) {
        ...
        playIcon.innerHTML = '<svg width="28" height="28" viewBox="0 0 24 24" fill="white"><path d="M8 5v14l11-7z"></path></svg>';
  ...
  DOM: {
    ...
    injectGlobalComponents: function() {
      ...
      if (nav) {
        nav.innerHTML = '<div class="nav-container"><a href="/" class="nav-logo">SUMIT UBALE</a><div class="nav-menu"><a href="/#portfolio" class="nav-link">Everything</a><a href="/pages/albums.html?s=all" class="nav-link">Albums</a><a href="/#about" class="nav-link">About</a><a href="/#contact" class="nav-cta">Enquire</a></div><button class="nav-toggle" aria-label="Toggle navigation"><span class="nav-toggle-line"></span><span class="nav-toggle-line"></span></button></div>';
      }

      var footer = document.getElementById('main-footer');
      if (footer) {
        footer.innerHTML = '<div class="container"><p>&copy; ' + new Date().getFullYear() + ' Sumit Ubale Photography. All rights reserved.</p></div>';
      }
    }
  }
};
