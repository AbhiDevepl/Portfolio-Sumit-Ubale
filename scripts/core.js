  ...
  Lightbox: {
    ...
    bindEvents: function() {
      ...
      document.addEventListener('keydown', function(e) {
        ...
        if (e.key === 'ArrowLeft') _this.nav(-1);
        if (e.key === 'ArrowRight') _this.nav(1);
        if (e.key === ' ') {
          e.preventDefault();
    ...
    bindTouch: function() {
      ...
      mediaContainer.addEventListener('touchstart', function(e) {
        if (!_this.state.active || e.touches.length !== 1) return;
        _this.state.touchActive = true;
        _this.state.touchStartX = e.touches[0].clientX;
        _this.state.touchCurrentX = e.touches[0].clientX;
      }, { passive: true });

      mediaContainer.addEventListener('touchmove', function(e) {
        if (!_this.state.touchActive || e.touches.length !== 1) return;
        _this.state.touchCurrentX = e.touches[0].clientX;
      }, { passive: true });
    ...
    open: function(index, items) {
      ...
      requestAnimationFrame(function() { _this.state.container.classList.add('active'); });
    ...
    updateContent: function() {
      ...
      requestAnimationFrame(function() {
        ...
        counterEl.textContent = (_this.state.currentIndex + 1) + ' / ' + _this.state.items.length;
      });
    }
  },
  ...
  Media: {
    createItem: function(image, index, allItems, categoryFormatter) {
      ...
      const openFilteredLightbox = function() {
        ...
        var visibleItems = fallbackItems;
