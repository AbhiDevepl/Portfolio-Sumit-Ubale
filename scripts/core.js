...
window.Core = {
  ...
  Media: {
    createItem(image, index, allItems, categoryFormatter = null, options = {}) {
      ...
      // Skip event handlers for improved performance when using event delegation (e.g., Homepage)
      if (options.skipHandler) return item;

      // Click logic: Videos toggle play on click (handled by VideoHover),
      // but open Lightbox when clicking their overlay. Images open Lightbox on click.
      if (isVideo) {
        overlay.onclick = (e) => {
          e.stopPropagation(); // Prevent video play toggle
