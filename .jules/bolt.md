...
## 2026-08-02 - [Coalescing Promises to Avoid Duplicate Concurrent HTTP Requests and Repetitive JSON Parses]
**Learning:** Even with sessionStorage caching, concurrent callers can trigger duplicate simultaneous HTTP fetch requests if they invoke the loader before the first response writes to the sessionStorage. Furthermore, calling `sessionStorage.getItem` and `JSON.parse` repetitively for large files like `portfolio.json` (~297KB) is synchronous and consumes precious CPU cycles during initialization. Caching the active/completed Promise itself in a private, module-scoped cache guarantees absolute request deduplication and bypasses `JSON.parse` entirely on subsequent calls, while maintaining storage fallback across page transitions.
**Action:** Always wrap data fetches in a module-scoped, IIFE-protected Promise cache that stores the Promise itself rather than the resolved value to perfectly deduplicate simultaneous requests and completely bypass repetitive decodes/parsing.
