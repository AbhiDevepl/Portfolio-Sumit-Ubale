...
## 2026-08-01 - [Deduplicating Concurrent Requests and Avoiding Synchronous Web-Storage Parse Overhead via Promise-Based Caching]
**Learning:** Although sessionStorage avoids network roundtrips on page reload/transition, synchronous calls to `sessionStorage.getItem` and `JSON.parse` block the main thread and incur measurable parsing overhead for large JSON files (e.g., 291KB portfolio data) when invoked multiple times in parallel or sequentially. By caching the active promise (`_portfolioDataPromise` and `_servicesDataPromise`) inside the window context, concurrent requests are automatically deduplicated. If a promise fails, a `.catch` block handles resetting the promise to ensure eventual consistency.
**Action:** Always use Promise-level in-memory caching in data-fetching abstractions to safely deduplicate concurrent requests and eliminate redundant parse/web-storage lookups during a single page lifecycle.
