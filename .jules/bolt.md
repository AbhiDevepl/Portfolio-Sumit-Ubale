...
## 2026-08-10 - [Synchronous CSS Variable Injection in Head to Prevent Flash of Unstyled Content (FOUC) and Layout Reflows]
**Learning:** Delaying the injection of design tokens (CSS custom properties) until `DOMContentLoaded` or checking `document.readyState === 'loading'` when the injection script is loaded synchronously in the `<head>` causes the browser to parse and render elements with unstyled layout fallback first. Once `DOMContentLoaded` fires, injecting the CSS variables triggers a major layout recalculation, paint pass, and visible Flash of Unstyled Content (FOUC). By executing the injection synchronously in the `<head>`, CSS variables are ready before DOM parsing begins, completely avoiding FOUC and expensive post-load reflows.
**Action:** Always inject global style variables synchronously in the `<head>` if the script is blocking, rather than waiting for DOM parsing to finish.
