...
## 2026-08-01 - [Synchronous Execution of Header Scripts for Critical CSS Properties Injection]
**Learning:** Deferring CSS custom properties/variables injection dynamically inside `<head>`-loaded scripts to `DOMContentLoaded` triggers a Flash of Unstyled Content (FOUC). This delay forces the browser to layout and paint using fallback styles first, then perform a costly full-page reflow and style recalculation when the variables are finally injected. Executing the style injection immediately upon script execution takes advantage of parser-time evaluation, ensuring style rules are registered inside `<head>` before any body content is parsed or painted.
**Action:** Inject theme variables, palette custom properties, and style overrides synchronously during script execution when loaded in `<head>`, rather than deferring to DOMContentLoaded or window.onload, to guarantee a single smooth first-paint cycle.
