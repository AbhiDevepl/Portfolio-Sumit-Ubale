...
## 2026-08-16 - [Promise-Based In-Memory Request Deduplication and JSON.parse Overhead Prevention]
**Learning:** Even when assets are cached in `sessionStorage`, concurrent calls to data fetchers (`Core.fetchPortfolioData` / `Core.fetchServicesData`) before storage is populated lead to duplicate parallel network requests for the ~291KB portfolio dataset. Furthermore, reading from `sessionStorage` on every fetch call requires executing `JSON.parse` on large payloads repeatedly, causing CPU thread blocking. Storing pending and resolved fetch promises in top-level private module variables (`_portfolioDataPromise` and `_servicesDataPromise`) deduplicates in-flight network requests and provides instant O(1) in-memory resolution for subsequent calls.
**Action:** Always wrap data fetch utilities with in-memory promise variables to deduplicate concurrent calls and eliminate redundant `JSON.parse` parsing overhead for large JSON assets.
