...
## 2026-08-01 - [Lazy Evaluation of Fallback Data in Event Handlers]
**Learning:** `Core.Media.createItem` was eagerly allocating and mapping 1,000+ objects into `fallbackItems` on every item click *before* checking if `window.GalleryManager.getVisibleData` was present. Because `GalleryManager` was always present on gallery pages, 1,000+ object allocations were performed and immediately discarded on every single click, causing unnecessary garbage collection pressure and interaction lag.
**Action:** Always lazy-evaluate expensive fallback structures inside event handlers behind conditional checks (`if/else`) so that calculations for fallback paths are skipped when primary handlers exist.
