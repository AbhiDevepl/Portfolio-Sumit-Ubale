...
## 2026-08-17 - [Eliminating Duplicate Category Event Handlers and Unconditional Fallback Allocations]
**Learning:** Having multiple scripts (`content-loader.js` and `gallery.js`) attaching direct click listeners to the same category buttons caused duplicate handler executions and double rendering passes on every tab switch. Additionally, unconditionally mapping 1,000+ items into `fallbackItems` before checking if `GalleryManager.getVisibleData()` was present wasted memory allocations on every item click.
**Action:** Audit event handlers for duplicate bindings when modular scripts co-exist on the same page, and always evaluate fallback data lazy-style to prevent garbage collection pressure during frequent user interactions.
