...
## 2026-07-23 - [Progressive Batch Loading & State Subscription in Portfolio Gallery]
**Learning:** Rendering 1,000+ items into the DOM synchronously during category switching or initial load of `pages/portfolio.html` causes a massive DOM size, rendering lag, layout thrashing, and blocks the main thread. Implementing a state-subscribed progressive renderer (batches of 36) using an `IntersectionObserver` sentinel decreases the initial DOM size by ~96.4%, and by ensuring that the absolute index in the fully filtered array is passed to the Media Factory, Lightbox pagination is kept fully decoupled from DOM size. Additionally, ScrollTrigger reveal animations should only selectively animate newly appended items, rather than re-triggering existing ones.
**Action:** Always favor progressive batch rendering (e.g. `IntersectionObserver` sentinels) for any list of size > 100 on high-frequency pages to minimize DOM overhead while keeping full list metadata pagination integrated with Lightboxes.
