...
## 2026-08-12 - [Defensive Smooth Scrolling and Third-Party API Guards]
**Learning:** Third-party scrolling engines (such as Lenis) and animation libraries (such as GSAP / ScrollTrigger) are not guaranteed to be loaded on every page or might fail to load over CDNs. Attempting to immediately instantiate or integrate them without robust defensive checks throws blocking runtime reference errors, halting execution of other scripts (like dynamic portfolio gallery loaders). Wrapping initialization scripts with `typeof` guardrails and offering standardized browser fallback behaviors completely prevents failures while optimizing UX.
**Action:** Always ensure that optional third-party integrations (like Lenis, GSAP, and ScrollTrigger) are wrapped defensively with typeof checks, and implement browser-native fallbacks (like smooth scrolling anchor link fallbacks) to keep the app responsive and error-free.
