...
## 2026-08-08 - [Defensive Smooth Scroll Fallback for Multi-Page Performance Stability]
**Learning:** Shared script assets like `scripts/smooth-scroll.js` may be loaded across pages that do not import heavy external libraries such as `Lenis`, `gsap`, or `ScrollTrigger` (e.g., `pages/portfolio.html`). If the script directly instantiates or connects to these libraries without safety checks, it throws a blocking `ReferenceError`, stopping subsequent JS execution. Wrapping the initialization in robust `typeof` guards and providing native `scrollIntoView` fallbacks ensures universal script compatibility and performance stability.
**Action:** Always verify if global utility scripts depend on optional CDN assets, and use defensive checks to provide lightweight, native browser fallbacks when those assets are absent.
