...
## 2026-09-05 - [Caching Static Lightbox DOM References to Eliminate High-Frequency DOM Traversals]
**Learning:** High-frequency interaction events in the Lightbox (such as `timeupdate`, play/pause toggles, and slide transitions) were repeatedly querying static DOM elements inside `#lightbox` using `querySelector` and `querySelectorAll`. Pre-caching these static DOM element references during `Core.Lightbox.init()` into `this.elements` completely eliminates redundant DOM tree traversals and layout thrashing during animations.
**Action:** When creating singleton UI engines (such as modal previewers, custom media controls, or lightboxes), always cache static internal DOM element references on initialization instead of querying the container on every user interaction.
