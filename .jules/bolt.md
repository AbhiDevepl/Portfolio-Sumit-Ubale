...
## 2026-08-05 - [Deduplicating Concurrent Data Fetches with In-Memory Promise Caching]
**Learning:** When multiple page scripts or loaders trigger `window.Core.fetchPortfolioData()` or `window.Core.fetchServicesData()` simultaneously during page startup, repeated calls invoke `sessionStorage.getItem()` and expensive `JSON.parse()` on the ~291KB portfolio string, or send multiple redundant HTTP requests if storage is empty. Storing in-memory promise references (`_portfolioDataPromise` and `_servicesDataPromise`) without the `async` wrapper keyword allows all concurrent callers to share a single identical Promise reference, completely eliminating redundant network requests and JSON parsing overhead.
**Action:** Use in-memory Promise caching for global data fetching utilities, and return the underlying Promise directly from non-async methods to preserve identical Promise reference equality across concurrent callers.
