...
## 2026-08-04 - [In-Memory Promise Caching & Request Deduplication for Data Fetches]
**Learning:** While `sessionStorage` avoids repeated network requests across page navigations, calling `sessionStorage.getItem()` and `JSON.parse()` on large JSON strings (~291KB) on every function call incurs noticeable CPU parsing overhead. Additionally, concurrent initializations of multiple components can trigger duplicate in-flight network requests before `sessionStorage` is populated. Adding in-memory promise memoization (`_portfolioDataPromise` and `_servicesDataPromise`) inside an IIFE in `scripts/core.js` deduplicates concurrent fetches and provides O(1) instant object access within the current page lifecycle.
**Action:** Combine `sessionStorage` with in-memory Promise caching for data utility functions to eliminate duplicate in-flight network requests and avoid repeated JSON parsing on the main thread.
