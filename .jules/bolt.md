...
## 2026-08-18 - [In-Memory Promise Deduplication to Prevent Concurrent Network Fetches & Redundant Deserialization]
**Learning:** Multiple modules and initializers across the codebase request portfolio or services data simultaneously or in sequence during startup. Relying solely on `sessionStorage` still incurs redundant `getItem` calls and heavy `JSON.parse()` deserialization overhead on a 291KB payload. Wrapping data fetchers (`fetchPortfolioData` and `fetchServicesData`) with in-memory promise deduplication (`_portfolioDataPromise` and `_servicesDataPromise`) eliminates duplicate in-flight requests and avoids repeated JSON parsing completely.
**Action:** Pair `sessionStorage` caches with an in-memory pending Promise reference to deduplicate concurrent requests and avoid redundant `JSON.parse` overhead during single page execution.
