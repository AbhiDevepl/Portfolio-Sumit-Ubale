...
## 2026-08-04 - [Lazy Randomized Probing for Cover Image Selection]
**Learning:** Selecting a random cover image from a large dynamic collection by applying O(N) `.filter()` and regular expression operations across hundreds of array items (such as category-wise images) is highly inefficient and creates significant memory/GC pressure. Since target items (e.g. non-video images) are highly prevalent in the collection, applying an O(1) lazy randomized probing approach (with a low, constant number of attempts) yields a success rate of >95% on the first try. A simple, fast sequential `.find()` can act as a fallback to guarantee a result if randomized probing fails, preventing full-array manipulation.
**Action:** When a random valid item is required from a large collection containing mostly valid items, avoid filtering the entire list first. Utilize a constant-time randomized probe followed by an early-exiting sequential lookup.
