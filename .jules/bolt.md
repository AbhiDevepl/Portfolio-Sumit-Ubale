...
## 2026-08-01 - [Preconnecting External Origins to Reduce Latency on Dynamic Subpages]
**Learning:** Dedicated subpages like `/pages/gallery.html` make heavy use of dynamic libraries (GSAP from `unpkg.com`), customized typefaces (Google Fonts), and dynamically generated portfolio items from external CDNs/Workers (`a.demo-developer596.workers.dev`). Without `preconnect` link tags, the browser waits until these scripts and stylesheets are parsed before initiating connection handshakes. Adding `<link rel="preconnect">` for these critical third-party domains allows the browser to perform DNS resolution, TCP handshake, and TLS negotiation in the background.
**Action:** Always leverage `preconnect` and `dns-prefetch` resource hints on pages loading remote assets or dynamic data endpoints to eliminate up to hundreds of milliseconds of connection startup latency.
