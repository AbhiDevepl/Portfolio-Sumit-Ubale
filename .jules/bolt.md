...
## 2026-08-04 - [Caching Lightbox DOM Element References and Lazy Fallback Allocation]
**Learning:** High-frequency interaction paths like Lightbox navigation (`updateContent`, `updatePlayPauseIcons`, `updateMuteButton`) were executing 7+ `querySelector`/`querySelectorAll` calls on every slide transition. Additionally, `openFilteredLightbox` eagerly computed shallow copies of all 1,000+ portfolio items (`fallbackItems`) on every item click even when `GalleryManager.getVisibleData` was present. Caching Lightbox element references in `this.elements` and evaluating `fallbackItems` lazily eliminates unnecessary DOM queries during slide transitions and prevents 1,000+ object allocations on item clicks.
**Action:** Always cache DOM element references on component instances for modal/lightbox engines that undergo frequent transition events, and lazily evaluate fallback array transformations to avoid allocations when custom manager data is available.
