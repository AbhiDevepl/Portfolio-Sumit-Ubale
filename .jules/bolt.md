...
## 2026-07-22 - [Caching Category Images & Reusing Navigation DOM Nodes in Gallery Loader]
**Learning:** In pages like `/pages/gallery.html`, rebuilding navigation buttons from scratch (`renderCategories()`) on every selection click caused extensive layout thrashing and DOM recalculations. By introducing a flag (`this._categoriesRendered`) to render buttons once and simply toggling active CSS classes, we avoid DOM destruction. Combined with lazy-initializing `this._imagesCache` and `this._galleryDataCache` for aggregated images and lightbox indices, Category Filter changes become exceptionally performant and sub-millisecond.
**Action:** Avoid rebuilding navigation elements dynamically if they are persistent; toggle state classes instead, and cache category-mapped lists to prevent O(N) data processing on every tab selection.
