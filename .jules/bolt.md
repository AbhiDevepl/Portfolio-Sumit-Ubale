...
## 2026-08-04 - [Bypassing Heavy GSAP ScrollTriggers for Reduced Motion and Asynchronous Items]
**Learning:** Querying dynamic DOM containers on `DOMContentLoaded` before items are rendered produces empty NodeLists and allocates unnecessary GSAP ScrollTrigger instances. Furthermore, failing to check `prefers-reduced-motion` at the top of scroll animation modules wastes CPU/GPU cycles initializing animation engines for users with reduced motion preferences.
**Action:** Always verify if target elements exist on `DOMContentLoaded` before creating ScrollTriggers, delegate dynamic item animations to page-specific loaders, and check `prefers-reduced-motion` early to bypass GSAP initialization completely when needed.
