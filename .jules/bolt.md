...
## 2026-08-01 - [Optimizing Album Cover Random Image Selection via Lazy Probing]
**Learning:** Picking a random image from a category to serve as its cover on `/pages/albums.html` was previously implemented via `.filter()` and regex matching on the entire category array (which contains hundreds of items). By replacing this O(N) full filter and regex search with an O(1) randomized probing loop (trying up to 5 random index attempts), we completely avoid array allocations and redundant regex testing on 1,000+ items. In the rare case where no random image is found in 5 attempts, a fallback `.find()` is used, which terminates as soon as a single match is found.
**Action:** Avoid full array filtering (`.filter()`) when only a single element is required. Instead, use randomized probing or early-terminating scans (`.find()`) to keep operations at constant or minimal linear complexity.
