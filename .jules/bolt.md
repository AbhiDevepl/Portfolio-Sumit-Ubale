...
## 2026-08-09 - [Defensive Third-Party Integration and Native Fallback in Shared Scripts]
**Learning:** Dynamic layout files like `smooth-scroll.js` are loaded globally or on multiple subpages. However, not all pages include external assets like `GSAP`, `Lenis`, or `ScrollTrigger` (e.g. `pages/portfolio.html`). Referencing these globals without safe `typeof` checks causes fatal `ReferenceError: Lenis is not defined` exceptions that halt script execution entirely. Guarding third-party bindings and implementing robust native-browser scrolling with offset calculations prevents runtime failures and maintains an optimal user experience.
**Action:** Always defensively wrap third-party library initializations and interactions in shared scripts with `typeof !== 'undefined'` checks and provide native fallback strategies where appropriate.
